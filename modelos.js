...
{id:"caja-pizza",nombre:"Caja pizza",cat:"cajas"},
{id:"caja-pillow",nombre:"Caja almohada",cat:"cajas"},
{id:"caja-taza",nombre:"Caja taza",cat:"cajas"},
{id:"caja-vela",nombre:"Caja vela",cat:"cajas"},
{id:"caja-jabon",nombre:"Caja jabón",cat:"cajas"},
...
function cajaCruz(w,h,d,op={}){
 let k=op.kerf||0,flap=op.flap||14,top=op.top||0,bottom=op.bottom||0;
 ...
 let cut=rect(x,y,w,h)+rect(x-d,y,d,h)+rect(x+w,y,d,h)+rect(x,y-d,w,d)+rect(x,y+h,w,d)+poly([[x+w+d,y+8],[x+w+d+flap,y+16],[x+w+d+flap,y+h-16],[x+w+d,y+h-8]]);
...
function cajaCupcake(w,h,d,op={}){let base=cajaCruz(clamp(w,65,95),clamp(h,60,100),clamp(d,45,70),{...op,top:d,bottom:d,flap:14}),ww=clamp(w,65,95),dd=clamp(d,45,70),hh=clamp(h,60,100),p=centrar(ww+dd*2+14,hh+dd*4),x=p.x+dd,y=p.y+dd*2;return agregar(base,circle(x+ww/2,y+hh/2,ww*.23))}
